function SimpleUploadAdapterPlugin(editor){
	editor.plugins.get( 'FileRepository' ).createUploadAdapter = function( loader ) { 
		...
		return new MyUploadAdapter( loader, "/file/introduce",window.csrf_name,window.csrf_val ); 
	};
	...
	upload() { 
		return this.loader.file
		 .then( file => new Promise( ( resolve, reject ) => { 
			 if(file.size>5*1024*1024){return reject("이미지가 5MB를 초과 했습니다.");}
			 this._initRequest(); 
			...
			editor.editing.view.change(
				function(writer){
					if(window.mobile){
						...
						writer.setStyle('max-height', '420px', editor.editing.view.document.getRoot());
						...
						writer.setStyle('max-height', '670px', editor.editing.view.document.getRoot());
					}
	...
	$.post(post_url,$("#"+frm).serialize(),function(res){
		...
		if(data.result=="Y"){
			Toast.show("등록 되었습니다.",{timeOut:1000, callback:function(){
				eval($("#"+frm+" input[name=callback]").val());
