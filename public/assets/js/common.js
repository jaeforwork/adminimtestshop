if (typeof(COMMON_JS) == 'undefined') {
    ...
    jQuery.fn.checkicon = function(init, unchecked, check) {
        ...
        $(this).find('> input:checkbox').change(function() {
            ...
            $(this).siblings('span').prop('class', 'glyphicon glyphicon-' + icon);
...
/*추가*/
function auto_next_focus( sObj1, sLength, sObj2 ) {
           if( sObj1.value.length == sLength )  document.getElementsByName(sObj2)[0].focus();
    }
//모든 팝업 함수 
function popup(Url,width,height){   
    win = window.open(Url,"window","resizable=no,width="+width+",height="+height+",left=0,top=0,scrollbars=yes");
    if(win) {   win.focus();}   
}
